...
class postgresql(
                  ...
                  $initdb                          = true,
                  $overcommit_memory               = '2',
                  $shmmax                          = ceiling(sprintf('%s', $facts['memory']['total_bytes'])*824633720832),
                  $shmall                          = ceiling(ceiling(sprintf('%s', $facts['memory']['total_bytes'])*824633720832)/$facts['eyp_postgresql_pagesize']),
                  ...
                  $wal_buffers                     = '-1',
                  $work_mem                        = '8MB',
                  $shared_buffers                  = sprintf('%dMB',ceiling(sprintf('%s', $facts['memory']['total_bytes'])*262144)),
                  $lc_messages                     = 'C',
                  ...
                  $effective_cache_size            = sprintf('%dMB',ceiling(sprintf('%s', ($facts['memory']['total_bytes'])*262144)*3)),
                  $wal_compression                 = true,
                  $log_line_prefix                 = undef,
                  $log_lock_waits                  = true,
                  $deadlock_timeout                = '1s',
                  $default_transaction_read_only   = false,
                  $max_worker_processes            = $facts['processors']['count'],
                  $max_parallel_workers            = $facts['processors']['count'],
                  $max_parallel_workers_per_gather = max(2, ceiling(sprintf('%.0f', ($facts['processors']['count'])/2))),
                  $hot_standby                     = false,
